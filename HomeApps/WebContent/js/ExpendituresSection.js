function ExpendituresSection(id, callbackObjs, appData) {
...
ExpendituresSection.prototype = new WuiComponent;

ExpendituresSection.prototype.getElementDef = function() {
	var initElements = {tag:'div',className:'expenses',elements:wui.toElements(
...
ExpendituresSection.prototype.refresh = function() {
...
ExpendituresSection.prototype.editExpenditureRec = function(w3cEvent) {
...
ExpendituresSection.prototype.cbNewExpenditureRec = function(w3cEvent) {
	var newExpenditureRec = this.appData.newTransaction(this.appData.getSelectedAccountIdx(), 0, new Date(), '', 'Expenditures', 'Basic', '', null, '');
	this.components.frmExpenditure.edit(newExpenditureRec, {mode:'new'});
};

ExpendituresSection.prototype.updateRecord = function(expenditureRec, token) {
	switch (token.mode) {
	case 'new':
...
ExpendituresSection.prototype.deleteExpenditureRec = function(w3cEvent) {
	var tr = w3cEvent.currentTarget.parentNode.parentNode;
	var expenditureRec = tr.userData.record;
...
ExpendituresSection.prototype.saveRepeat = function(expenditureRec, repeatDtl, token) {
	switch (token.mode) {
	case 'new':
...
ExpendituresSection.prototype.saveRepeatAuto = function(expenditureRec, token) {
	switch (token.mode) {
	case 'new':
		this.appData.addTransaction(expenditureRec);
		break;
